...
public class GenerateNewConfirmTokenCommandHandler : IRequestHandler<GenerateNewConfirmTokenCommand, ErrorOr<GenerateNewConfirmTokenCommandResponse>>
{
    ...
    public GenerateNewConfirmTokenCommandHandler(UserManager<NormalUser> normalUserManager, IEmailServiceSender emailServiceSender, ILogger<GenerateNewConfirmTokenCommandHandler> logger, IEmailTemplateService emailTemplateService, IOtpGenerator otpGenerator)
    {
    ...
    {
        _logger.LogInformation("Generating new email confirmation token for user with email {Email}", request.Email);
        ...
        _logger.LogInformation("New email confirmation token generated for user with email {Email}", request.Email);
