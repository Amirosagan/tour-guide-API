...
public class ResetPasswordQueryHandler : IRequestHandler<ResetPasswordQuery, ErrorOr<ResetPasswordQueryResponse>>
{
    ...
    public ResetPasswordQueryHandler(UserManager<NormalUser> normalUserManager, UserManager<TourGuide> tourGuideManager, ILogger<ResetPasswordQueryHandler> logger, IOtpGenerator otpGenerator)
    {
    ...
    public async Task<ErrorOr<ResetPasswordQueryResponse>> Handle(ResetPasswordQuery request, CancellationToken cancellationToken)
    {
        ...
        if (user != null)
        {
            ...
            var result = await _normalUserManager.ResetPasswordAsync(user, token, request.NewPassword);
            
            if (result.Errors.Any())
            {
                _logger.LogError("Error resetting password for user with email {Email}", request.Email);
        ...
        {
            var token = _otpGenerator.VerifyOtp(request.Otp);
            var result = await _tourGuideManager.ResetPasswordAsync(tourGuide, token, request.NewPassword);
            
            if (result.Errors.Any())
            {
                _logger.LogError("Error resetting password for tour guide with email {Email}", request.Email);
                return Error.Failure(description: result.Errors.Select(x => x.Description).Aggregate((x, y) => $"{x}, {y}"));
            }
            
            _logger.LogInformation("Password reset for tour guide with email {Email}", request.Email);
            return new ResetPasswordQueryResponse();
        }
